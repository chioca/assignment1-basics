...
def run_embedding(
    ...
    """
    from cs336_basics.transformer.utils import Emebedding

    layer = Emebedding(num_embeddings=vocab_size, embedding_dim=d_model)
    with torch.no_grad():
...
def run_multihead_self_attention(
    ...
    with torch.no_grad():
        atten_layer.q_proj.weight.copy_(q_proj_weight)
        atten_layer.k_proj.weight.copy_(k_proj_weight)
        atten_layer.v_proj.weight.copy_(v_proj_weight)
        atten_layer.out_proj.weight.copy_(o_proj_weight)
    return atten_layer(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    with torch.no_grad():
        ...
        atten_layer.v_proj.weight.copy_(v_proj_weight)
        atten_layer.out_proj.weight.copy_(o_proj_weight)
    return atten_layer(in_features, token_positions)
...
def run_rope(
    ...
    """
    from cs336_basics.transformer.utils import RoPE
...
def run_transformer_block(
    ...
    block.load_custom_state_dict(weights)
...
def run_transformer_lm(
    ...
    """
    from cs336_basics.transformer.transformer import transformer_llm
    ...
    llm.load_weight_from_dict(weights)
...
def run_rmsnorm(
    ...
    """
    from cs336_basics.transformer.utils import RMSNorm
...
def get_adamw_cls() -> Any:
    ...
    """
    from cs336_basics.train.utils import AdamW
